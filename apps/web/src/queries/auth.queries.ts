...
export function useRegister() {
  ...
  return useMutation(
    async (dto: { organizationName: string; firstName: string; lastName: string; email: string; password: string }) => {
      const { data } = await api.post("/auth/register", dto);
      return data.data;
    },
    {
      onSuccess: () => {
        dispatch(addToast({ type: "success", message: "Registration successful! Please check your email." }));
        navigate("/login");
      },
      onError: (error: { response?: { data?: { error?: { message?: string } } } }) => {
        dispatch(addToast({
