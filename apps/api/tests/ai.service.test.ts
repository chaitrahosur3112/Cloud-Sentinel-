...
const fakeForecastResult = {
  forecast: [{ date: "2026-07-10", predicted: 150, lower: 120, upper: 180 }],
  trend: "stable",
  percentChange: 2.5,
};
...
describe("AiService.forecastResourceCost", () => {
  it("throws 404 when resource not found", async () => {
    ...
    await expect(service.forecastResourceCost("bad-id", "org1"))
  ...
  it("throws 422 when fewer than 14 days of history", async () => {
    mockRepo.getDailyHistoryForResource.mockResolvedValueOnce({
      resource: { id: "r1", name: "prod-vm" },
      rows: makeDays(5),  // only 5 days — not enough
    } as never);

    await expect(service.forecastResourceCost("r1", "org1"))
      .rejects.toThrow(new AppError("Not enough cost history", 422));
  });
...
describe("AiService.detectCostAnomalies", () => {
  it("throws 422 when fewer than 10 data points", async () => {
    mockRepo.getAllResourceDailyHistory.mockResolvedValueOnce(makeDays(3) as never);
    await expect(service.detectCostAnomalies("org1"))
      .rejects.toThrow(new AppError("Not enough cost data", 422));
  });

  it("returns only flagged anomalies in the response", async () => {
    mockRepo.getAllResourceDailyHistory.mockResolvedValueOnce(makeDays(30) as never);
    mockMlClient.detectAnomalies.mockResolvedValueOnce({
      anomalies: [
        { date: "2026-06-01", amount: 500, resourceId: "r1", isAnomaly: true,  anomalyScore: 0.9 },
        { date: "2026-06-02", amount: 100, resourceId: "r1", isAnomaly: false, anomalyScore: 0.1 },
      ],
      totalAnomalies: 1,
      anomalyRate: 3.33,
    });
